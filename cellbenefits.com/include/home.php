...
if ($featured_phones != ""){
	// Make sure they are all still available (not discontinued)
	$aScratch = explode(",", $featured_phones);
	$aFeatured = array();
	for ($counter=0; $counter < count($aScratch); $counter++){
		$query = "SELECT * FROM phones WHERE product_id = '".$aScratch[$counter]."'";
		$rs_valid = mysql_query($query, $linkID);
		$row = mysql_fetch_assoc($rs_valid);
		if ($row["display"] != "N"){
			// Not discontinued, so push it onto the stack
			array_push($aFeatured, $aScratch[$counter]);
		}
	}
//print_r($aFeatured);
//echo '<br>';
	// Set aside the phones defined in $aFeatured
	$restriction = "";
	for ($counter=0; $counter < count($aFeatured); $counter++){
		$restriction .= " AND product_id <> '".$aFeatured[$counter]."'";
	}
	// if the list is shorter than 3 add more to the end
	if (count($aFeatured) < 3){
		if (!$carrier_selected){
			$query = "SELECT * FROM phones WHERE display = 'F' AND (carrier = 'Nextel' OR carrier = 'Sprint') ".$restriction." ORDER BY rand() LIMIT ".(3-count($aFeatured));
		}else{
			$query = "SELECT * FROM phones WHERE display = 'F' AND carrier = '$carrier_selected' ".$restriction." ORDER BY rand() LIMIT ".(3-count($aFeatured));
		}
//echo $query."<br>";
		$rs_extras = mysql_query($query, $linkID);
		// Push the new items onto the array stack
		for ($counter=1; $counter <= mysql_num_rows($rs_extras); $counter++){
			$row = mysql_fetch_assoc($rs_extras);
			array_push($aFeatured, $row["product_id"]);
		}
	}
	// Shake it up
	shuffle($aFeatured);
//print_r($aFeatured);
	// Build query to get the first 3 off the top
	$query = "SELECT * FROM phones WHERE product_id = '".$aFeatured[0]."'";
	for ($counter=1; $counter < 3; $counter++){
		$query .= " OR product_id = '".$aFeatured[$counter]."'";
	}
//echo $query;
	...
	if ($sprint_site == "T" || $nextel_site == "T" ){
		if (!$carrier_selected){
			$rs_featured = mysql_query("SELECT * FROM phones WHERE display = 'F' AND (carrier = 'Nextel' OR carrier = 'Sprint') ORDER BY rand() LIMIT 3", $linkID);
		}else{
			$rs_featured = mysql_query("SELECT * FROM phones WHERE display = 'F' AND carrier = '$carrier_selected' ORDER BY rand() LIMIT 3", $linkID);
		}
	// All others
	}else{
		$rs_featured = mysql_query("SELECT * FROM phones WHERE display = 'F' AND carrier = '$carrier_selected' ORDER BY rand() LIMIT 3", $linkID);
		...
		if ($row["mail_in".$pricing_level."-1"] != 0 || $row["mail_in".$pricing_level."-2"] != 0){
			echo'
						<tr>
							...
							<td>&nbsp;mail-in rebate'.iif($row["mail_in".$pricing_level."-1"] != 0 && $row["mail_in".$pricing_level."-2"] != 0, "s", "").iif($carrier_selected == "AT&T", "&dagger;", "").'</td>
						</tr>
			';
		}
//		if ($cingular_site == "T" && $site == "yahoo"){
...
if ($featured_phones != "" && $aFeatured[3] != ""){
//print_r($aFeatured);
//echo '<br>';
	// Set aside the phones we already displayed above
	$restriction = "";
	for ($counter=0; $counter < count($aFeatured); $counter++){
		$restriction .= " AND product_id <> '".$aFeatured[$counter]."'";
	}
//echo $restriction."<br>";
	// If the list is shorter than 9 (these 6 plus the previous 3) get more
	if (count($aFeatured) < 9){
		if (!$carrier_selected){
			$query = "SELECT * FROM phones WHERE display = 'F' AND (carrier = 'Nextel' OR carrier = 'Sprint') ".$restriction." ORDER BY rand() LIMIT ".(9-count($aFeatured));
		}else{
			$query = "SELECT * FROM phones WHERE display = 'F' AND carrier = '$carrier_selected' ".$restriction." ORDER BY rand() LIMIT ".(9-count($aFeatured));
		}
//echo $query."<br>";
		$rs_extras = mysql_query($query, $linkID);
		// Push them onto the end of the array stack
		for ($counter=1; $counter <= mysql_num_rows($rs_extras); $counter++){
			$row = mysql_fetch_assoc($rs_extras);
			array_push($aFeatured, $row["product_id"]);
		}
...
//print_r($aFeatured);
//echo '<br>';
	// Now flip the array, pop off the bottom 3 (prior top 3, used above), then flip it back (flipping back not really necessary, but it's tidy)
	$aReversed = array_reverse($aFeatured);
	array_pop($aReversed);
	array_pop($aReversed);
	array_pop($aReversed);
	$aFeatured = array_reverse($aReversed);
//print_r($aFeatured);
//echo '<br>';
	// Shuffle what's left
	shuffle($aFeatured);
//print_r($aFeatured);
	// Build query to get phones from where we left off
	$query = "SELECT * FROM phones WHERE product_id = '".$aFeatured[0]."'";
...
//echo $query;
	$rs_promoed = mysql_query($query, $linkID);
}else{
	// Sprint-Nextel sites
	if ($sprint_site == "T" || $nextel_site == "T" ){
		if (!$carrier_selected){
