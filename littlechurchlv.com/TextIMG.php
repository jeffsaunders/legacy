...
dropcap		- Image is a dropcap, 'yes' or 'no'?
...
// Determine image width needed to hold string
function getWidth($fontSize, $angle, $fontFace, $string, $offset, $buffer){
    $ret = "";
	$testbox = imagettfbbox($fontSize, $angle, $fontFace, $string);
	$ret = $testbox[2] + $offset + $buffer;
	return $ret;
}

// Determine image height needed to hold string
function getHeight($fontSize, $angle, $fontFace, $string, $offset, $buffer){
    $ret = "";
	$testbox = imagettfbbox($fontSize, $angle, $fontFace, $string);
	$ret = $testbox[3] + $offset + $buffer;
	return $ret;
}

// Draw text bolded
function drawboldtext($image, $size, $angle, $x_cord, $y_cord, $r, $g, $b, $fontfile, $text, $style){
...
//echo $font;
$bold = $_REQUEST['bold']; //Bold 'yes', 'no', 'extra' (Extra Bold), or 'super' (Extra Extra Bold)
if ($bold != "yes" && $bold != "extra" && $bold != "super") $bold = "no";
...
$dropcap = $_REQUEST['dropcap']; //Is this image a dropcap? 'yes' or 'no'
if ($dropcap != "yes") $dropcap = "no";
...
// Create the image
if ($dropcap == "yes"){
	$img = imagecreatetruecolor(getWidth($points, $angle, $font, $text, $left, 5), getHeight($points, $angle, $font, $text, $top, 0));
}else{
...
if ($shadow !=""){
	if ($bold == "yes" || $bold == "extra" || $bold == "super"){
		drawboldtext($img, $points, $angle, $left+$shadowOffset, $top+abs($shadowOffset), hexdec('0x'.$shadow{0}.$shadow{1}), hexdec('0x'.$shadow{2}.$shadow{3}), hexdec('0x'.$shadow{4}.$shadow{5}), $font, wrap($points, $angle, $font, $text, $width, $left),$bold);
	}else{
		imagettftext($img, $points, $angle, $left+$shadowOffset, $top+$shadowOffset, $shadowColor, $font, wrap($points, $angle, $font, $text, $width, $left));
	}
}

// Now stir in the text
if ($bold == "yes" || $bold == "extra" || $bold == "super"){
	drawboldtext($img, $points, $angle, $left, $top, hexdec('0x'.$txtcolor{0}.$txtcolor{1}), hexdec('0x'.$txtcolor{2}.$txtcolor{3}), hexdec('0x'.$txtcolor{4}.$txtcolor{5}), $font, wrap($points, $angle, $font, $text, $width, $left),$bold);
}else{
