...
$query="SELECT * FROM handicap WHERE id=2";
...
if(isset($_GET['success']) && strlen($_GET['member'])==32) {
    ...
    $query="SELECT * FROM subscription WHERE md5(CONCAT('ffc_',user_id))='".$_GET['member']."' AND handicapper=2";
    $result=mysql_query($query);
    if($result && mysql_num_rows($result)) {
        ...
        $result=mysql_query($query);
        $data=@mysql_fetch_array($result);
        $query="INSERT INTO subscription SET `date`=CURDATE(), user_id='".$data['login_id']."', handicapper=2, type='".$subscription_status."', expire=CURDATE() + INTERVAL ".$subscription_length."";
        mysql_query($query);
    }
    //echo $query;
...
<head>
<title>www.footballforecast.com Dennis Tobler's Football Forecast Weekly. Predictions. Football Handicapping.</title>
<meta name="keywords" content="sports betting info, Sports Gambling, football forecast weekly, football broadcast, Free Sports Picks, Free NFL football picks, SPORTS  PICKS, NFL picks, free picks, Football, handicapper info, free college football picks, handicapping, football shows">
<meta name="description" content="Dennis Tobler's Football Forecast Weekly, Free Sports Picks, handicapping software">
...
?>
	<table border="0" width="600" cellspacing="1" cellpadding="5" id="table1" bgcolor="#C0C0C0" height="88">
		...
		<tr style="font-size: 10pt;">
			...
			?>
			<div align="center">
			<input onclick="alert('Please login to order picks.');location.href='login.php?url=hc2.php';" type="button" name="pay" value="Buy Picks of The Week from <?php echo $data['name'];?> for $99"><br /><br />
			<input onclick="alert('Please login to order picks.');location.href='login.php?url=hc2.php';" type="button" name="pay" value="Buy All Picks from <?php echo $data['name'];?> for $199">
			</div>
...
/*
&return=http%3A%2F%2Fwww.footballforecast.com%2Fhc2.php&cancel_return=http%3A%2F%2Fwww.footballforecast.com%2Fhc2.php%3Ffail%3D1&notify_url=http%3A%2F%2Fwww.footballforecast.com%2Fipn.php&custom=handicapper-1,member-<?php echo $_SESSION['login_id']?>
&return=http%3A%2F%2Fwww.footballforecast.com%2Fhc2.php&cancel_return=http%3A%2F%2Fwww.footballforecast.com%2Fhc2.php%3Ffail%3D1&notify_url=http%3A%2F%2Fwww.footballforecast.com%2Fipn.php&custom=handicapper-1,member-<?php echo $_SESSION['login_id']?>
*/
?>	
	</div>
...
<img border="0" src="images/spacer_content.gif" width="180" height="1" alt="football, baseball, basketball bets, edge, sports, Las Vegas, Nevada, nfl, mlb, nhl, nba, lines, pointspreads, point spreads, odds, sport service, teasers, sportbooks, sportsbooks, services, gaming, selections, NFL picks weekly, nfl football picks, betting, handicapping odds"><br>
