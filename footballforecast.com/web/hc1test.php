...
?>
	<table border="0" width="600" cellspacing="1" cellpadding="5" id="table1" bgcolor="#C0C0C0" height="88">
		...
		<tr style="font-size: 10pt;">
			...
			?>
			<div align="center">
			<input onclick="window.location.href='https://www.paypal.com/cgi-bin/webscr?custom=1,<?php echo md5('ffc_'.$_SESSION['login_id']);?>&notify_url=<?php echo urlencode('http://www.footballforecast.com/ipn.php')?>&cmd=_xclick&business=billing%40footballforecast%2ecom&item_name=Pick%20Of%20The%20Week<?php echo urlencode('-'.$data['name']); ?>&item_number=pow&no_shipping=1&no_note=1&amount=0%2e01&no_shipping=0&no_note=1&currency_code=USD&return=<?php echo urlencode('http://www.footballforecast.com/hc1.php?success=1&type='.md5('pow').'&member='.md5('ffc_'.$_SESSION['login_id']));?>'" type="button" name="pay" value="Buy Picks of The Week from <?php echo $data['name'];?> for $99"><br /><br />
			<input onclick="window.location.href='https://www.paypal.com/cgi-bin/webscr?custom=1,<?php echo md5('ffc_'.$_SESSION['login_id']);?>&notify_url=<?php echo urlencode('http://www.footballforecast.com/ipn.php')?>&cmd=_xclick-subscriptions&business=billing%40footballforecast%2ecom&item_name=All%20Picks%20Subscription<?php echo urlencode('-'.$data['name']); ?>&item_number=pick_subscription&no_shipping=1&no_note=1&currency_code=USD&lc=US&a3=0%2e01&p3=1&t3=M&src=1&sra=1&return=<?php echo urlencode('http://www.footballforecast.com/hc1.php?success=1&type='.md5('full').'&member='.md5('ffc_'.$_SESSION['login_id']));?>'" type="button" name="pay" value="Buy All Picks from <?php echo $data['name'];?> for $199">
			<form action=""></form>
			</div>
			<?php
			}
...
WESTERN UNION</A><br>
<A HREF="javascript:void(0)" onClick="window.open('mg.php', 'MyPopUp', 'width=432,height=270,toolbar=0,scrollbars=0,screenX=200,screenY=200,left=200,top=200')">
MONEY GRAM</A></font></b><br>
&nbsp;</div>
