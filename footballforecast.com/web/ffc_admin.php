			...
			if(isset($_POST['name'])) {
				if($_POST['id']>0) {
					...
					if(is_file($_FILES['photo']['tmp_name']) && $_FILES['photo']['name']<>'') {
						if(move_uploaded_file($_FILES['photo']['tmp_name'],'images/handicapper/'.$_FILES['photo']['name'])) {
							//insert new data
							$query="INSERT INTO handicap SET `date`=CURDATE(), name='".$_POST['name']."', photo='images/handicapper/".$_FILES['photo']['name']."', 
  ...
  <table width="500" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC" align="center" style="font: arial; font-size: 10pt;">
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td><select name="status" id="status">
        <option value="tv_multimedia" '.$status['tv_multimedia'].'>TV Multimedia</option>
...
';
		...
		case 'multimedia': {
			//handle file deletion
			if(isset($_GET['delete'])) {
				$query="DELETE FROM movie WHERE movie='".$_GET['delete']."'";
				mysql_query($query);
				//delete file physically
				@system('rm '.$_GET['delete']);
			}
			//handle file upload
			if(isset($_POST['title'])) {
				if((int)$_POST['id']>0) {
					//update movie
					$query="UPDATE movie SET title='".$_POST['title']."', description='".$_POST['description']."', 
					status='".$_POST['status']."', movie='".$_POST['movie']."' WHERE id='".$_POST['id']."'";
					mysql_query($query);							
					if(mysql_affected_rows()>0) {
						$message='Video data updated';
					}
					...
					//move uploaded file
					$query="INSERT INTO movie SET title='".$_POST['title']."', description='".$_POST['description']."', 
					movie='".$_POST['movie']."', status='".$_POST['status']."'";
					mysql_query($query);
					if(mysql_affected_rows()>0) {
						$message='New video added to database';
					}
					else {
						$message='Unable to save video to database. Please try again later';
					}
			...
			//get file list from multimedia folder
			$video=array();
			$movie=array();
			$dir=opendir('mov/multimedia');
			if($dir) {
				while ($file=readdir($dir)) {
					if($file<>'.' && $file<>'..' && is_file('mov/multimedia/'.$file)) {
						$video[]='mov/multimedia/'.$file;
					}
				}
			}
			$dir=opendir('mov/archive');
			if($dir) {
				while ($file=readdir($dir)) {
					if($file<>'.' && $file<>'..' && is_file('mov/archive/'.$file)) {
						$video[]='mov/archive/'.$file;
					}
				}
			}
			foreach ($video as $val) {
				$query="SELECT * FROM movie WHERE movie='".$val."'";
				$result=mysql_query($query);
				$data=array();
				if($result && mysql_num_rows($result)>0) {
					$data=mysql_fetch_array($result);
  ...
  <tr bgcolor="#FFFFFF">
    ...
    <td>'.$data['movie'].'</td>
    <td align="center">'.$data['status'].'</td>
    <td width="50" align="center"><a title="Edit '.$data['title'].'" href="?action=multimedia&edit='.urlencode($data['movie']).'"><img src="images/edit.gif" border="0"></a></td>
    <td width="50" align="center"><a title="Delete '.$data['title'].'" onClick="return confirm(\'Delete '.addslashes($data['title']).'?\');" href="?action=multimedia&delete='.urlencode($data['movie']).'"><img src="images/delete.gif" border="0"></a></td>
  </tr>
';					
			}
			...
			//edit movie
			$data=array();
			$query="SELECT * FROM movie WHERE movie='".$_GET['edit']."'";
			$result=mysql_query($query);
			if($result && mysql_num_rows($result)>0) {
				$data=mysql_fetch_array($result);
				$status[$data['status']]='selected';
...
<br /><hr>
<form action="?action=multimedia" method="post" enctype="multipart/form-data" name="form1">
<div align="center" style="font: arial; font-size: 10pt; color: #FF0000;">'.$message.'</div>
  <table width="500" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC" align="center" style="font: arial; font-size: 10pt;">
    <tr align="center" bgcolor="#EEEEEE">
      <td colspan="2"><strong>'.$form_title.'</strong></td>
    ...
    <tr bgcolor="#FFFFFF">
      <td>Video File</td>
      <td><input name="movie" type="text" id="movie" size="35" value="'.$data['movie'].'"></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td>Status</td>
      <td><select name="status" id="status">
...
<table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#000000">
  <tr align="center" bgcolor="#9C0000">
    <td class="admin_menu" style="border:1px solid #C0C0C0; font-family: Arial; font-size: 8pt; font-weight: bold"><a href="?action=multimedia" class="admin_menu">
