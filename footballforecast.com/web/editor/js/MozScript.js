...
function wp_context(obj, evnt) {
	...
	if (n >= 1) {
		...
		document.onmousedown=wp_closePopupTimer
	}
...
function wp_editor(obj,config) {
	...
	if (str.search(/<body/gi) != -1) {
		...
		if (obj.stylesheet != '') {
			str += '<link rel="stylesheet" href="'+obj.stylesheet+'" type="text/css">'
		}
	} else {
		obj.snippit = true
...
function wp_send_to_edit_object(obj, init) {
	...
	if ((!obj.snippit) && (obj.html_edit_area.value != '')) {
			...
			if (n>1) {
				...
				for (i=0; i<n; i++) {
					if (attribute_array[i].search("=") != -1) {
						...
						var elm = attribute[0].trim().replace(/"/,'')
						var val = attribute[1].trim().replace(/"/,'')
		...
		var headtag = obj.edit_object.document.getElementsByTagName('HEAD')
		if (obj.stylesheet != '') {
			var headcontent = obj.charset + obj.baseURL + '<link rel="stylesheet" href="' + obj.stylesheet + '" type="text/css">'
		} else {
