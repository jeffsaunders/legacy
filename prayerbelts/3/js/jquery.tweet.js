    ...
    return this.each(function(){
      ...
      $.getJSON(url, function(data){
        ...
        $.each(data.results, function(i,item){
		  ...
		  var username = '<a href="http://twitter.com/'+item.from_user+'/statuses/'+item.id+'" title="view tweet on twitter">'+item.from_user+'</a>';
          var text = '<span class="tweet_text">' +$([item.text]).linkUrl().linkUser().linkHash().makeHeart().capAwesome().capEpic()[0]+ '</span>';
          
          // until we create a template option, arrange the items below to alter a tweet's display.
          list.append('<li><blockquote>'+ text + '<i>' + date + '</i>'+ '</blockquote>'+ '</li>' + '<div class="reminder">Follow ' + username + ' on Twitter for more news and updates on items.</div>');
