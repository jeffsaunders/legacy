...
class DataAccessSQLite extends DataSQLite {
	...
	// aliases for field names to make them more readable
	private $fieldnameMap = array(  '_transactid_' => 'Transaction ID',
	...
	public function AttachTransActions ( $dbpath ) {
		...
		$this->transact = $this->_Exec ( 'ATTACH "' .  $dbpath . '" AS ' . TBL_TRANSACT );
	...
	function GetAllFields ( ) {
		...
		if( $this->outputfields === false ) {
			...
			if( $this->transact ) {
				...
				$morefields = $this->_GetTableFields( TTRANS );
	...
	public function GetItemCart ( $rowid )
	{
		...
		$qry = 'SELECT ' . $this->_EscapeName( $this->tablename ) . '.rowid, ' . TBL_TRANSACT . '.*'
			 . ' FROM ' . $this->_EscapeName( $this->tablename )
			 . ' LEFT JOIN ' . TBL_TRANSACT . '.' . TTRANS . ' ON ' . TTRANS . '.route = _transactid_'
	...
	private function _QrySelectFrom ( $tablename, $allfields = false ) {
		...
		foreach( $this->fields as $fld ) {
			...
			switch( $fld ) {
				...
				default:
					if( isset( $this->fieldnameMap[ $fld ] ) )
						$myfields .= ', ' . $this->_EscapeName( $fld ) . ' AS ' . $this->_EscapeName( $this->fieldnameMap[ $fld ] );
					else
		...
		if( $this->transact )
			$qry .= ' LEFT JOIN ' . TBL_TRANSACT . '.' . TTRANS . ' ON ' . TTRANS . '.route = _transactid_'; 
