...
define( 'MC_URL', 'http://%dc%.api.mailchimp.com/1.3/' );
...
class MailChimp extends PageExtension {
		...
		foreach( $this->cfg->lists as $mclist ) {
			...
			foreach( $mclist->action as $action => $condition ) { 
				...
				if( $valid &&	
					...
					$this->$action( $mclist ) )						return true;
			}
		}
	...
	private function subscribe ( $mclist ) {
		...
		$this->_contactMailChimp( 'listSubscribe' );
	...
	private function unsubscribe ( $mclist ) {
		...
		$this->_contactMailChimp( 'listUnsubscribe' );
		if( ! $this->result || !is_object( $this->result ) )				return false;
	...
	private function update ( $mclist ) {
		...
		$this->_mergeTags( $mclist->merge_tags );
		$this->_contactMailChimp( 'listUpdateMember' );
		if( ! $this->result || !is_object( $this->result ) )				return false;
	...
	private function _prepareData ( $action, $config ) {
		...
		if( isset( $config->$action->double_optin ) )			$this->data[ 'double_optin' ] = $config->$action->double_optin;
	...
	private function _contactMailChimp ( $method ) {
		...
		if( ! empty($this->merges) )			$this->data['merge_vars'] =& $this->merges;
		...
		$ch = curl_init();
		curl_setopt( $ch, CURLOPT_URL, $url .'?method=' . $method );
	...
	private function _mergeTags ( $merge_tags, $subset = false ) {
		...
		foreach( $merge_tags as $tag ) {
			...
			if( ! isset( $tag->field_type ) ||
				! isset( $tag->fb_name ) ||
				! method_exists( $this, $tag->field_type ) ) {
			...
			// size test
			if( isset( $tag->size ) &&
				is_string( $this->getPost( $tag->fb_name ) ) &&
				strlen( $this->getPost( $tag->fb_name ) ) > $tag->size ) {
				
				$this->setError( _T( 'Field %s can\'t have a value with more that %s characters.', array( $tag->fb_name, $tag->size ) ) );
				continue;
			}
	...
	private function grouping ( $tag ) {
		...
		// escape ',' in the group names
		array_walk( $input, array( $this, '_escapeGroup'));

		// ensure the array exists and add this entry
		if( ! isset( $this->merges[ 'GROUPINGS' ] ) )		$this->merges[ 'GROUPINGS' ] = array();

		$this->merges[ 'GROUPINGS' ][] = array( 'name' => $tag->tag,
												'groups' => implode(',', $input ) );
	}
	...
	*/
	private function address ( $tag ) {
		...
		foreach( $keys as $key ) { if( ! isset( $adr[ $key ] ) ) $adr[ $key ] = ''; }
