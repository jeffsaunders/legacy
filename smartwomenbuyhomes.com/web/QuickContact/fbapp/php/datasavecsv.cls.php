...
class DataSaveCSV extends DataSave {
	...
	function _MakeNew ( ) {
		...
		foreach( Config::GetInstance()->GetReservedFields() as $name )
		{
			if( ! in_array( $name, $this->ignoreFields ) )		$columns[] = $name;
		}
	...
	function _CheckFields ( ) {
		...
		return array_values( $delta) == array_values( array_diff( Config::GetInstance()->GetReservedFields(), $this->ignoreFields ) );
	}
