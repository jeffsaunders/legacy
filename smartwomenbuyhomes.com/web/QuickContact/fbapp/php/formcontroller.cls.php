...
class FormController  {
	...
	private function _ShowConfirmPage ( )
	{
		...
		if( $custom_html ) {
			...
			$url = preg_replace( '/[^\/]*?$/', FormPage::GetInstance()->GetFormName(true) . '/confirm.html', $_POST[ CC_FB_CUSTOMHTML ] );
			header( 'Location: ' .  $url );
	...
	private function _ShowInlinePage ( )
	{
		...
		if( ! empty( $html ) ) {
			...
			if( $userdom->loadHTML( '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">' . $html ) ) {
	...
	private function _PreparePayment ( )
	{
		...
		Config::GetInstance()->SetSessionVariable( 'code', FormPage::GetInstance()->source );
		Config::GetInstance()->SetSessionVariable( 'post', serialize( FormPage::GetInstance()->post ) );
