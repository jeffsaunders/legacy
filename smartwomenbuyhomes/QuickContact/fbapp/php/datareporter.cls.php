...
class DataReporter {
	...
	private $maxwidth = -1;
	private $db = false;
	private $transacts = false;				// set true if the transaction db is attached
	...
	function GetItemData ( )
	{
		...
		{
			if( in_array( strtolower( $this->db->GetRealName( $name ) ), $amountfields ) )
			{
				$this->data['data'][ $name ] = $this->curSign . number_format( $value * $this->divider, $this->decimals );
			}
		}
	...
	private function _PostProcessData ( )
	{
		...
		if( ! is_array( $this->data['data'] ) ||
			(empty( $formatfields ) && empty( $amountfields ) && $this->maxwidth == -1 ) )	return;
		...
		for( $i = 0; $i < count( $this->data['data'] ); $i++ )
		{
			foreach( $this->data['data'][ $i ] as $key => &$value )
			{
				...
				if( in_array( strtolower( $realName ), $amountfields ) && ! empty( $value )  )
				{
	...
	{
		$amountfields = array();
		$amountfields[] = $this->db->GetRealName( 'grandtotal' );
