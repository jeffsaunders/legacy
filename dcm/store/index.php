...
// if just displaying cart contents in upper-right, then bail and go get just that.
if ($_REQUEST["page"] == "infoBox"){
	(isset($_SESSION['customer_id']) ? $name = $_SESSION['customer_first_name']." ".$_SESSION['customer_last_name'] : $name = "Guest");
	header('Location: /store/infoBox.php?cart='.$_SESSION['cart']->count_contents().'&name='.$name); die();
}
