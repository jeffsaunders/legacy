<!-- BEGIN Include activate_tieline.php -->
...
?>
	...
	//			document.PlanForm.IMEI.value = "";
		...
		<!-- Step Label -->
			...
			<tr>
				<td class="superbigBlack">Activate with <? echo iif($site == "premier", "PWS", "Tieline"); ?></td>
			</tr>
			</table>
	...
	<!--		<div id="WelcomeMessage" style="position:static; border-top:1px solid <? echo $header_bgcolor; ?>; visibility:visible;">-->
		...
		?>
				<table width="900" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="superbigBlack">Welcome to Tieline</td>
				</tr>
				<tr>
					<td class="bodyBlack"><br>
						Tieline Technology, a leading provider of high-quality remote broadcast digital audio codecs, has carved out a strong reputation in international broadcast and industrial audio and video markets. &nbsp;The Tieline solution is easy to use, works with a variety of transmission mediums, sounds great on the air (and doesn't conflict with the HD Radio algorithm,) and most importantly will stand the tests of time and the road.<br><br>

						Tieline operates under a well defined strategic business plan which will continue to deliver innovative solutions to professional audio and video markets worldwide. &nbsp;The services associated with 3G provide the ability to transfer both voice data (a phone call) and non-voice data (such as downloading information, exchanging email and instant messaging).<br><br>

						Cellular activations and service plans are the thread that ties these 3G, UMTS, and EVDO enabled products onto the network. &nbsp;This activation portal gives customers the tools to remotely access and easily activate the service providers directly. &nbsp;Through this process the customer is given the unique ability to establish service accounts, as well as activate lines, without having to track down network salespeople and explain the M2M market in order to get up and running.<br><br>
 		 			</td>
				</tr>
				</table>
		<?
	...
	?>
	<!--		<div id="CDMADevices" style="position:static; border-top:1px solid <? echo header_bgcolor; ?>; visibility:<? echo iif(($locked && ($order['carrier'] == "sprint" || $order['carrier'] == "verizon")), 'visible;', 'hidden; display:none;'); ?>">-->
...
//							}
				...
				<tr>
					<td width="900">
						<?
						$query = "SELECT * FROM plans WHERE carrier = '".$order['carrier']."' ORDER BY display_order ASC";
	...
	<!--						<td align="center"><input type="text" name="ESN<? echo $counter; ?>" id="ESN<? echo $counter; ?>" size="8" maxlength="8" style="width:80px; background-color:<? echo $form_bg; ?>;" onKeyUp="if (this.value.length == 8){return checkESN('<? echo $counter; ?>');};" onChange="checkESN('<? echo $counter; ?>');" value=""> <!-- 4 hex digits (8 char.) --</td>-->
							<td align="center"><input type="text" name="ESN<? echo $counter; ?>" id="ESN<? echo $counter; ?>" size="11" maxlength="20" style="width:80px; background-color:<? echo $form_bgcolor; ?>;" value="<? echo $ESN[$counter]; ?>"> <!-- 4 hex digits (8 char.) or 11 digits decimal --></td>
							<td align="center">
							...
							<td align="center"><input type="text" name="RequestAreaCode<? echo $counter; ?>" id="RequestAreaCode<? echo $counter; ?>" size="15" maxlength="50" class="bodyBlack" style="width:80px; background-color:<? echo $form_bgcolor; ?>;" onChange="popACs('<? echo $counter; ?>','<? echo $order['device_count']; ?>');" value="<? echo $RequestAreaCode[$counter]; ?>"></td>
							<td align="center"><input type="text" name="User<? echo $counter; ?>" id="User<? echo $counter; ?>" size="15" maxlength="50" class="bodyBlack" style="width:180px; background-color:<? echo $form_bgcolor; ?>;" value="<? echo $User[$counter]; ?>"></td>
						</tr>
						<?
						}
						?>
...
<!--								<input type="button" name="AddButton" value="Add" onClick="document.PlanForm.sec.value = 'activate2'; document.PlanForm.submit();" style="position:relative;">&nbsp;&nbsp;&nbsp;-->
					...
					?>
						...
						<tr>
							<td class="xbigBlack">Sprint Plans:<br></td>
						</tr>
						<tr>
						<?
						...
						<tr>
							...
							<?
							$query = "SELECT * FROM plans WHERE carrier = 'Sprint' AND display = 'T' ORDER BY display_order ASC";
							$rs_plans = mysql_query($query, $linkID);
							echo'
					...
					?>
						...
						<tr>
							<td class="xbigBlack">Verizon Plans:<br></td>
						</tr>
						<tr>
						<?
						...
						<tr>
							<td width="930" height="15">
							<?
							$query = "SELECT * FROM plans WHERE carrier = 'Verizon' AND display = 'T' ORDER BY display_order ASC";
							$rs_plans = mysql_query($query, $linkID);
							echo'
								<table width="900" border="1" cellspacing="0" cellpadding="0" align="center" bordercolor="'.$border_bgcolor.'" class="bodyBlack">
								<tr bgcolor="'.$border_bgcolor.'" class="bodyWhite">
				...
				function validateGSM() {
					...
					for (counter=1; counter <= <? echo $order['device_count']; ?>; counter++){
						if (eval('document.PlanForm.ICCID'+counter+'.value') != ""){
							...
							for (counter2=1; counter2 <= counter-1; counter2++){
								if (eval('PlanForm.ICCID'+counter2+'.value == PlanForm.ICCID'+counter+'.value')){
									eval('PlanForm.ICCID'+counter2+'.style.background="#FFE100";');
									eval('PlanForm.ICCID'+counter+'.style.background="#FF0000";');
									alert('ICCID Already Entered For Device #'+counter2);
									eval('PlanForm.ICCID'+counter2+'.style.background="#FFFFFF";');
									eval('PlanForm.ICCID'+counter+'.style.background="#FFFFFF";');
									eval('PlanForm.ICCID'+counter+'.focus();');
									return false;
								}
							...
							for (counter2=1; counter2 <= counter-1; counter2++){
								if (eval('PlanForm.IMEI'+counter2+'.value == PlanForm.IMEI'+counter+'.value')){
									eval('PlanForm.IMEI'+counter2+'.style.background="#FFE100";');
									eval('PlanForm.IMEI'+counter+'.style.background="#FF0000";');
									alert('IMEI Already Entered For Device #'+counter2);
									eval('PlanForm.IMEI'+counter2+'.style.background="#FFFFFF";');
									eval('PlanForm.IMEI'+counter+'.style.background="#FFFFFF";');
									eval('PlanForm.IMEI'+counter+'.focus();');
									return false;
								}
...
//					document.PlanForm.submit();
	...
	<!--					<tr bgcolor="<? echo $box_color; ?>" style="background-image:url(images/ButtonBarBG.jpg);" class="bodyWhite">-->
						...
						<tr bgcolor="<? echo $box_bgcolor; ?>" class="bodyBlack">
							...
							?>
							<td align="center"><input type="text" name="ICCID<? echo $counter; ?>" id="ICCID<? echo $counter; ?>" size="20" maxlength="20" style="width:150px; background-color:<? echo $form_bgcolor; ?>;" value="<? echo $ICCID[$counter]; ?>"> <!-- 20 digits decimal --></td>
							<td align="center"><input type="text" name="IMEI<? echo $counter; ?>" id="IMEI<? echo $counter; ?>" size="15" maxlength="15" style="width:110px; background-color:<? echo $form_bgcolor; ?>;" value="<? echo $IMEI[$counter]; ?>"> <!-- 15 digits decimal --></td>
							<td align="center">
							...
							<td align="center"><input type="text" name="RequestAreaCode<? echo $counter; ?>" id="RequestAreaCode<? echo $counter; ?>" size="15" maxlength="50" class="bodyBlack" style="width:60px; background-color:<? echo $form_bgcolor; ?>;" onChange="popACs('<? echo $counter; ?>','<? echo $order['device_count']; ?>');" value="<? echo $RequestAreaCode[$counter]; ?>"></td>
							<td align="center"><input type="text" name="User<? echo $counter; ?>" id="User<? echo $counter; ?>" size="15" maxlength="50" class="bodyBlack" style="width:80px; background-color:<? echo $form_bgcolor; ?>;" value="<? echo $User[$counter]; ?>"></td>
						</tr>
						<?
						}
						?>
...
<!--								<input type="button" name="AddButton" value="Add" onClick="document.PlanForm.sec.value = 'activate2'; document.PlanForm.submit();" style="position:relative;">&nbsp;&nbsp;&nbsp;-->
					...
					?>
						...
						<tr>
							<td class="xbigBlack">AT&T Plans:<br></td>
						</tr>
						<tr>
						<?
						...
						<tr>
							<td width="930" height="15">
							<?
							$query = "SELECT * FROM plans WHERE carrier = 'AT&T' AND display = 'T' ORDER BY display_order ASC";
							$rs_plans = mysql_query($query, $linkID);
							echo'
								<table width="900" border="1" cellspacing="0" cellpadding="0" align="center" bordercolor="'.$border_bgcolor.'" class="bodyBlack">
								<tr bgcolor="'.$border_bgcolor.'" class="bodyWhite">
...
<!--	<input type="hidden" name="DeviceCount" id="DeviceCount" value="">-->
...
//		}
		...
		if (theForm.password){
			var pin_regex = /^[1-9]\d{5}$/;  // nnnnnn
			if (pin_regex.test(theForm.password.value) == false){
				theForm.password.style.background="#FF0000";
				alert("Please Enter A Six Digit PIN Number");
		...
		if (theForm.security_question){
			if (theForm.security_question.value == "" && theForm.password.value == ""){
				theForm.security_question.style.background="#FF0000";
				theForm.security_answer.style.background="#FF0000";
				theForm.password.style.background="#FF0000";
		...
		// Verify Security Answer
		if (theForm.security_answer){
			if (theForm.security_question.value != "" && theForm.password.value == "" && theForm.security_answer.value == ""){
				theForm.security_answer.style.background="#FF0000";
				alert("Please Enter A Private Security Answer");
				theForm.security_answer.style.background="#FFFFFF";
				theForm.security_answer.focus();
				...
				<td>
					...
					<tr>
						<td width="800" valign="top">
							...
							<tr>
								<td valign="top" class="superbigBlack">Activate with <? echo iif($site == "premier", "PWS", "Tieline"); ?></td>
							</tr>
							<tr>
...
//										}
							...
							?>
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++; ?>">
								<td align="right" valign="top" class="xbigBlack"><img src="images/spacer.gif" alt="" width="1" height="3" border="0"><br><? echo $carrier; ?> Account Password:</td>
								<td>
									<table border="0" cellspacing="5" cellpadding="0" class="bodyBlack">
									<tr>
										<td width="650" valign="top">
											<input type="text" name="password" id="password" size="30" maxlength="50" tabindex="7" onKeyPress="return onlyNumbers(event,this);" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['password']; ?>"><span class="smallBlack"> *Must be a Six Digit Number<br>Enter Desired Password/PIN</span></td>
									</tr>
									</table>
								</td>
							</tr>
							...
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++;  ?>">
								...
								<td>
									<table border="0" cellspacing="5" cellpadding="0" class="bodyBlack">
									<tr>
										<td width="195" valign="top"><input type="text" name="wireless_phone" id="wireless_phone" size="30" maxlength="50" tabindex="50" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['wireless_phone']; ?>"><span class="smallBlack"><br>Primary Number for This Account</span></td>
...
//									$rowcount = 0;
							...
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); ?>">
								...
								<td>
									...
									<tr>
										<td colspan="2" class="xbigBlack"><img src="images/spacer.gif" alt="" width="1" height="20" border="0"><br><img src="images/spacer.gif" alt="" width="81" height="1" border="0">Account Password/PIN:<img src="images/spacer.gif" alt="" width="15" height="1" border="0"><input type="text" name="password" id="password" size="30" maxlength="50" tabindex="54" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['password']; ?>"><? echo iif($carrier == 'Sprint', '<span class="smallBlack"> *Enter Either or Both</span>', '<span class="smallBlack"> *If Applicable</span>'); ?></td>
									</tr>
									</table>
								</td>
							</tr>
							...
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); ?>">
								...
								<td>
									<table border="0" cellspacing="5" cellpadding="0" class="bodyBlack">
									<tr>
										<td colspan="2" class="xbigBlack"><img src="images/spacer.gif" alt="" width="81" height="1" border="0">Account Password/PIN:<img src="images/spacer.gif" alt="" width="15" height="1" border="0"><input type="text" name="password" id="password" size="30" maxlength="50" tabindex="55" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['password']; ?>"><? echo iif($carrier == 'Sprint', '<span class="smallBlack"> *Required</span>', '<span class="smallBlack"> *If Applicable</span>'); ?></td>
									</tr>
									</table>
								</td>
							</tr>
...
<!--								                <option value="Unknown"<? if($order["security_question"]=="Unknown") echo " selected";?>>Unknown/Uncertain</option>-->
									...
									<tr>
										<td colspan="2" class="xbigBlack"><img src="images/spacer.gif" alt="" width="57" height="1" border="0">Requested Password/PIN:<img src="images/spacer.gif" alt="" width="15" height="1" border="0"><input type="text" name="password" id="password" size="30" maxlength="50" tabindex="58" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['password']; ?>"><? echo iif($order['add_line'] == "Yes", '<span class="smallBlack"> *Optional</span>', '<span class="smallBlack"> *Required</span>'); ?></td>
									</tr>
									</table>
								</td>
							</tr>
							...
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++; ?>">
								...
								<td>
									...
									<tr>
										<td width="650">
											<input type="text" name="password" id="password" size="30" maxlength="50" tabindex="59" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;" value="<? echo $order['password']; ?>"><? echo iif($order['add_line'] == "Yes", '<span class="smallBlack"> *Optional</span>', '<span class="smallBlack"> *Required</span>'); ?>
										</td>
									</tr>
									</table>
								</td>
									...
									<tr>
										<td>
											<select name="shipping" id="shipping" tabindex="201" class="bodyBlack" style="width:200px; background-color: <? echo $form_bg; ?>;">
												<option value="">Select</option>
								                <option value="2-Day"<? if($order['shipping']=="2-Day") echo " selected";?>>2-Day for $9.95</option>
...
// Column width defined here - first row in ALL forms
							?>
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++; ?>">
								<td width="300" align="right" valign="top">Service Address:<br><span class="smallBlack"><strong>Primary Place of Use Address<br>Cannot be a P.O. Box.</strong></span></td>
								<td width="600">
									<strong><? echo $order['service_address_1']; echo iif($order["service_address_2"] != "", ", ", ""); echo $order['service_address_2']; ?><br>
									<? echo $order['service_city']; ?>,&nbsp;<? echo $order['service_state']; ?>&nbsp;&nbsp;<? echo $order['service_zipcode']; ?></strong>
								</td>
							...
							?>
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++; ?>">
								<td align="right" valign="top">Requested Password:</td>
								<td><strong><? echo $order['password']; ?></strong></td>
							</tr>
							<?
							}
							...
							?>
							<tr bgcolor="<? echo iif(is_even($rowcount), $roweven, $rowodd); $rowcount++; ?>">
								<td align="right" valign="top" class="bigBlack">Shipping Option:</td>
		...
		<!-- Step Label -->
			...
			<tr>
				<td>
					<table width="900" border="0" cellspacing="0" cellpadding="0" align="center">
					<tr>
						<td valign="top" class="xbigBlack">Thank You for Activating with <? echo iif($site == "premier", "PWS", "Tieline"); ?></td>
					</tr>
					<tr>
...
<!-- END Include activate_tieline.php -->
