...
function openTicket(){
	...
	$type = iif($_REQUEST['port_type'] == "", $_REQUEST['change_type'], $_REQUEST['port_type']);
	$query =
		"INSERT INTO tickets (
		session_id,
		opened_by,
...
//echo $query.'<br></br>';
	...
	$dest = $_REQUEST['port_type'];
	header("Location: $uri/?sec=port&task=$dest&status=$status");
	break;

case "edit_port":	// Edit an existing port ticket
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
		opened_by = '".$_REQUEST['opened_by']."',
		opened_for = '".$_REQUEST['opened_for']."'
		WHERE ticket_num = '".$_REQUEST['ticket_num']."'";
//echo $query.'<br></br>';
	$rs_edittkt = mysql_query($query, $linkID);
	// now write the new transfer record
	$query =
		"UPDATE transfers SET
		device = '".$_REQUEST['device']."',
		imei = '".$_REQUEST['imei']."',
		sim_icc = '".$_REQUEST['sim_icc']."',
		first_name = '".$_REQUEST['first_name']."',
		last_name = '".$_REQUEST['last_name']."',
		employee_id = '".$_REQUEST['employee_id']."',
		cost_center = '".$_REQUEST['cost_center']."',
		user_email = '".$_REQUEST['user_email']."',
		port_num = '".$_REQUEST['port_num']."',
		acct_type = '".$_REQUEST['acct_type']."',
		port_from = '".$_REQUEST['port_from']."',
		bill_name = '".$_REQUEST['bill_name']."',
		bill_address1 = '".$_REQUEST['bill_address1']."',
		bill_address2 = '".$_REQUEST['bill_address2']."',
		bill_city = '".$_REQUEST['bill_city']."',
		bill_state = '".$_REQUEST['bill_state']."',
		bill_zipcode = '".$_REQUEST['bill_zipcode']."',
		from_acct_num = '".$_REQUEST['from_acct_num']."',
		from_acct_pass = '".$_REQUEST['from_acct_pass']."',
		tax_id = '".$_REQUEST['tax_id']."',
		requester_name = '".$_REQUEST['requester_name']."',
		requester_phone = '".$_REQUEST['requester_phone']."',
		requester_email = '".$_REQUEST['requester_email']."'
		WHERE ticket_num = '".$_REQUEST['ticket_num']."'";
//echo $query.'<br></br>';
	$rs_editport = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Port Request Updated';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	$ticket = $_REQUEST['ticket_num'];
	header("Location: $uri/?sec=ticket&task=view_transfer&cargo=transfers|$ticket&status=$status");
	break;

case "approve_port":	// Update ticket - set approved to true
	session_start(); 
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
		...
		approved_time = DATE_SUB(NOW(), INTERVAL 2 HOUR)
		WHERE ticket_num = '".$_REQUEST['ticket_num']."'";
//echo $query.'<br></br>';
	$rs_edittkt = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Port Request Approved';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	header("Location: $uri/?sec=ticket&task=list_approvals&status=$status");
	break;

case "disapprove_port":	// Update ticket - close ticket without approving
	session_start(); 
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
		approved = 'F',
		closed = 'T',
		closed_by = '".$_SESSION['user']."',
		closed_time = DATE_SUB(NOW(), INTERVAL 2 HOUR),
		ticket_resolution = 'Disapproved - ".$_REQUEST['reason']."'
		WHERE ticket_num = '".$_REQUEST['ticket_num']."'";
//echo $query.'<br></br>';
	$rs_edittkt = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Port Request Disapproved and Closed';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	header("Location: $uri/?sec=ticket&task=list_approvals&status=$status");
	break;

case "add_port_note":	// Update ticket - add a note
	session_start(); 
	// Add note
	$query =
		...
		'".$_REQUEST['new_note']."')";
//echo $query.'<br></br>';
	$rs_addportnote = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Note Added';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
...
//	$_SESSION['SID'] = session_id();
	...
	$query =
		...
		'".$_REQUEST['change_data_plan']."',
		'".$_REQUEST['new_data_plan']."',
...
case "edit_change":	// Edit an existing change ticket
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
		opened_by = '".$_REQUEST['opened_by']."',
...
//echo $query.'<br></br>';
	...
	$query =
		...
		user_email = '".$_REQUEST['user_email']."',
		device = '".$_REQUEST['device']."',
		current_num = '".$_REQUEST['current_num']."',
		imei = '".$_REQUEST['imei']."',
		change_voice_plan = '".$_REQUEST['change_voice_plan']."',
		new_voice_plan = '".$_REQUEST['new_voice_plan']."',
		change_data_plan = '".$_REQUEST['change_data_plan']."',
		new_data_plan = '".$_REQUEST['new_data_plan']."',
		world_traveler = '".$_REQUEST['world_traveler']."',
		world_connect = '".$_REQUEST['world_connect']."',
		blackberry_data = '".$_REQUEST['blackberry_data']."',
		blackberry_intl = '".$_REQUEST['blackberry_intl']."',
		sim_icc = '".$_REQUEST['sim_icc']."',
		new_sim_icc = '".$_REQUEST['new_sim_icc']."',
		new_areacode = '".$_REQUEST['new_areacode']."',
		new_address1 = '".$_REQUEST['new_address1']."',
		new_address2 = '".$_REQUEST['new_address2']."',
		new_city = '".$_REQUEST['new_city']."',
		new_state = '".$_REQUEST['new_state']."',
		new_zipcode = '".$_REQUEST['new_zipcode']."',
		...
		WHERE ticket_num = '".$_REQUEST['ticket_num']."'";
//echo $query.'<br></br>';
	$rs_editchange = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Change Request Updated';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	$ticket = $_REQUEST['ticket_num'];
	header("Location: $uri/?sec=ticket&task=view_change&cargo=changes|$ticket&status=$status");
	break;

case "approve_change":	// Update ticket - set approved to true
	session_start(); 
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
...
case "disapprove_change":	// Update ticket - close ticket without approving
	session_start(); 
	// write changes to the ticket
	$query =
		"UPDATE tickets SET
...
//echo $query.'<br></br>';
	$rs_edittkt = mysql_query($query, $linkID);
	// Tell 'em what you did
	$status = 'Change Request Disapproved and Closed';
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	header("Location: $uri/?sec=ticket&task=list_approvals&status=$status");
	break;

case "add_change_note":	// Update ticket - add a note
	session_start(); 
	// Add note
	$query =
		"INSERT INTO ticket_notes (
...
//echo $query.'<br></br>';
	...
	// Send 'em back
	$uri = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
	header("Location: $uri/?sec=manage&task=list_devices&status=$status");
	break;
