...
$referers = array("blindcenter.org","www.blindcenter.org");
...
function send_mail($form,$invis_array){
  ...
  $mailbody = "The following Holiday Basket Order was submitted by:\n";
  $mailbody.= $form["billname"]. " (for " .$form["shipname"].") at " . date("g:ia o\\n F dS, Y") ."\n\n";
    ...
    for ($tobla = 0; $tobla < count($toarray); $tobla++){
      mail($toarray[$tobla],$form["subject"],$mailbody,"From: " .$form["billname"]. "\nX-Mailer: PHPFormMail (http://www.boaddrink.com)");
    }
}
...
if(check_referer($referers) && check_required($form)){
  ...
  fill_data($form["bill_phone"],"Blank Bill Phone Field");
  fill_data($form["billname"],$form["bill_first_name"]." ".$form["bill_last_name"]);
  fill_data($form["shipname"],$form["ship_first_name"]." ".$form["ship_last_name"]);
  fill_data($form["title"],"Order Results");
