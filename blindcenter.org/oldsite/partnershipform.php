...
function send_mail($form,$invis_array){
  ...
  $mailbody = "The following Corporate Partnership Program Registration Form was submitted by\n";
  $mailbody.= $form["submitter"]. " (" .$form["email"].") at " . date("g:ia o\\n F dS, Y") ."\n\n";
    ...
    for ($tobla = 0; $tobla < count($toarray); $tobla++){
      mail($toarray[$tobla],$form["subject"],$mailbody,"From: " .$form["email"]. " (" .$form["submitter"]. ")\nX-Mailer: PHPFormMail (http://www.boaddrink.com)");
    }
}
...
if(check_referer($referers) && check_required($form)){
  ...
  fill_data($form["email"],"Blank Email Field");
  fill_data($form["phone"],"Blank Phone Field");
  fill_data($form["submitter"],$form["contact"]." of ".$form["firm"]);
